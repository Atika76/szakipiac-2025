...
async function megjelenitHirdetesek() {
    ...
    try {
        ...
        hirdetesek.forEach(h => {
            const deleteButton = loggedInUserEmail === ADMIN_EMAIL ? `<button class="delete-btn" onclick="deleteAd(${h.id})">Törlés</button>` : '';
...
async function deleteAd(id) {
    ...
    if (error) {
        ...
        megjelenitHirdetesek(); // A lista frissítése törlés után
    }
}

// Eseménykezelők, hogy a szűrés működjön
