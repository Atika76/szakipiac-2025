const MUNKAFIGYELO_BUILD = "V6-DIREKT-ONCLICK-20260704-1905";
...
export function createMunkafigyelo({ client, showToast, trackEvent, adminEmail }) {
  ...
  async function loadPublicJobs() {
    ...
    if (error) {
      ...
      publicJobs = BEAGYAZOTT_KOZBESZERZESEK.filter(isUsablePublicJob);
      return { fallback: true, error };
    }
    const rows = ((data || []).length ? (data || []) : BEAGYAZOTT_KOZBESZERZESEK).filter(isUsablePublicJob);
    publicJobs = rows;
  ...
  function renderJobCards(target, jobs) {
      ...
      return `
        <article class="sp-card bg-white rounded-2xl border border-slate-200 shadow-sm p-5" data-job-id="${esc(job.id)}">
          ...
          <div class="mt-5 pt-4 border-t border-slate-100 flex flex-wrap items-center gap-2">
            <button type="button" data-details="${esc(job.id)}" class="bg-slate-100 text-slate-800 border border-slate-200 px-4 py-2.5 rounded-xl font-bold hover:bg-slate-200">Részletek</button>
    ...
    target.querySelectorAll("[data-details]").forEach(button => button.addEventListener("click", () => openDetails(button.dataset.details)));
  ...
  function openDetails(id) {
    const job = publicJobs.find(x => String(x.id) === String(id));
    if (!job) return showToast("A munka részletei nem találhatók.", "error");
    const sourceUrl = safeUrl(job.forras_url);
    showModal(`<h2 class="text-xl font-black">${esc(job.cim)}</h2>
      <div class="mt-3 text-sm text-slate-600 space-y-1">
        <p><b>Szakma:</b> ${esc(job.szakma)}</p>
        <p><b>Hely:</b> ${esc(job.telepules)}, ${esc(job.megye)}</p>
        <p><b>Keret:</b> ${esc(budgetText(job))}</p>
        ${job.kezdes_datum ? `<p><b>Kezdés:</b> ${esc(formatDate(job.kezdes_datum))}</p>` : ""}
        <p><b>Forrás:</b> ${esc(sourceLabel(job))}</p>
      </div>
      <div class="mt-4 rounded-xl bg-slate-50 border border-slate-200 p-4 whitespace-pre-line text-slate-800">${esc(job.leiras)}</div>
      <div class="mt-5 flex flex-wrap gap-2">
        ${job.kapcsolat_elerheto ? `<button type="button" data-details-contact="${esc(job.id)}" class="bg-emerald-700 text-white px-4 py-2.5 rounded-xl font-extrabold">Kapcsolatfelvétel</button>` : ""}
        ${sourceUrl ? `<a href="${esc(sourceUrl)}" target="_blank" rel="noopener noreferrer" class="bg-blue-50 text-blue-700 border border-blue-200 px-4 py-2.5 rounded-xl font-bold">Eredeti hirdetés ↗</a>` : ""}
      </div>
      ${(!job.kapcsolat_elerheto && !sourceUrl) ? `<p class="mt-4 text-sm text-rose-700 font-bold">Ehhez a munkához nincs elérhető kapcsolat vagy eredeti link, ezért nem jelenik meg éles találatként.</p>` : ""}`);
    root.querySelector("[data-details-contact]")?.addEventListener("click", () => { closeModal(); openContact(id); });
  }
  ...
  function formHtml(job = null) {
    ...
    return `
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 md:p-7">
        <div class="flex items-start justify-between gap-4 mb-6"><div><h2 class="text-2xl font-black">${job ? "Munka módosítása" : "Új munka feladása"}</h2><p class="text-sm text-slate-600 mt-1">30 napig ingyenesen megjelenik. A kapcsolati adataid rejtve maradnak.</p><div class="mt-3 rounded-xl border border-amber-300 bg-amber-50 text-amber-900 px-3 py-2 text-sm font-black">✅ Friss Munkafigyelő verzió aktív: ${esc(MUNKAFIGYELO_BUILD)}<br><span class="text-xs font-bold">Ha ezt a sárga sort látod, akkor az új javítás fut. Ha nem látod, még régi fájl van a böngészőben vagy a GitHub Pages-en.</span></div></div><span class="bg-emerald-100 text-emerald-800 rounded-full px-3 py-1 text-xs font-black">INGYENES</span></div>
        <form id="mf-job-form" novalidate data-edit-id="${esc(job?.id || "")}" class="space-y-5">
          ...
          <div data-mf-status class="hidden rounded-xl border p-3 text-sm font-bold"></div><div class="flex flex-wrap gap-3"><button type="button" data-mf-submit-job onclick="window.__mfDirectSaveFromButton && window.__mfDirectSaveFromButton(this)" class="bg-emerald-700 text-white px-6 py-3 rounded-xl font-black hover:bg-emerald-800 active:scale-95 disabled:opacity-60 disabled:cursor-wait transition">${job ? "Módosítás mentése" : "Munka közzététele"}</button>${job ? `<button type="button" data-cancel-edit class="px-5 py-3 rounded-xl border border-slate-300 font-bold">Mégse</button>` : ""}</div>
        </form>
      </div>`;
  }
  ...
  async function saveJob(formOrEvent) {
    ...
    try {
      ...
      if (editId) {
        ...
        const rpcResult = await client.rpc("munkafigyelo_munka_feladasa", {
          p_cim: payload.cim,
          p_leiras: payload.leiras,
          p_szakma: payload.szakma,
          p_megye: payload.megye,
          p_telepules: payload.telepules,
          p_iranyitoszam: payload.iranyitoszam,
          p_surgosseg: payload.surgosseg,
          p_koltseg_min: payload.koltseg_min,
  ...
  async function renderPushPanel(panel) {
    ...
    panel.querySelector("#mf-push-form button[type=submit]")?.addEventListener("click", event => {
      event.preventDefault();
      event.stopPropagation();
      savePush({ preventDefault() {}, currentTarget: panel.querySelector("#mf-push-form") });
    });
  ...
  if (typeof window !== "undefined") {
    window.__mfDirectSaveFromButton = function(button) {
      const form = button?.closest?.("#mf-job-form");
      if (!form) {
        alert("Munkafigyelő hiba: nem találom az űrlapot a gomb mellett.");
        return;
      }
      const box = form.querySelector("[data-mf-status]");
      if (box) {
        box.className = "rounded-xl border border-blue-200 bg-blue-50 text-blue-700 p-3 text-sm font-bold";
        box.textContent = "✅ A gomb kattintása biztosan érzékelve. Mentés indul…";
      }
      saveJob(form);
    };
  }
